...
import numpy as np
from ffd import util
from ffd.bernstein import bernstein_poly, trivariate_bernstein
...
def get_stu_control_points(dims):
    stu_lattice = util.mesh3d(*(np.linspace(0, 1, dims + 1, dtype=np.float32)
                                for d in range(dims)))
...
def get_stu_deformation_matrix(stu, dims):
    v = util.mesh3d(*(np.arange(0, dims + 1, dtype=np.int32)
                      for d in range(dims)))
    v = np.reshape(v, (-1, 3))
